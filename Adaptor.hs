...
theGoalAbs m absGame ops ipdb ivdb spdb svdb offset absState = do
    ...
    (goals, PredDBState{..}) <- runStateT (gameGoals absGame) state
    return $ GoalAbsRet dbStatePreds dbStateVars dbNextIndex (snd $ head goals) dbUserState
